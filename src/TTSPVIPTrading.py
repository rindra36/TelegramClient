...
class TTSPVIPTradingBot:
    ...
    async def handle_trade_execution(self, message: str) -> bool:
        ...
        if not self._has_complete_trade_data(channel):
            ...
            logging.info(f'TTSPVIPTrading.py {extracted_data}')
    ...
    async def _execute_new_trade(self, channel: str) -> bool:
        ...
        logging.info(f'TTSPVIPTrading.py Trade parameters: {self.pocket_option.get_channel_data(channel)}')

        try:
            return await self.execute_trade_cycle(channel)
        except Exception as e:
            logging.error(f'TTSPVIPTrading.py Trade cycle failed: {e}')
            return False
    ...
    async def execute_trade_cycle(self, channel: str) -> bool:
        ...
        """
        logging.info('TTSPVIPTrading.py INITIATE TRADE')
        ...
        self.pocket_option.remove_channel_data(channel)
        logging.info(f'TTSPVIPTrading.py TRADE PLACED SUCCESSFULLY: "{trade_id}"')
    ...
    async def _monitor_trade_result(self, trade_id: str, channel: str) -> bool:
        ...
        logging.info(f'TTSPVIPTrading.py Trade result: {trade_result}')

        if trade_result in ['win', 'draw']:
            logging.info(f'TTSPVIPTrading.py Trade successful: {trade_result}')
            return True

        logging.warning(f'TTSPVIPTrading.py Trade {trade_id} failed ({trade_result})')
        return False

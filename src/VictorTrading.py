...
class VictorTradingBot:
            ...
            if retry > 0 or await waiting_time(entry, timezone_offset=self.TIMEZONE_OFFSET):
                try:
                    ...
                    if not self.DO_MARTINGALE:
                        logging.warning(f'VictorTrading.py Martingale needed here but not activated. Stop on retry {retry}')
                        return False
                    retry += 1
                    amount *= 2
                    ...
                    logging.error(f'VictorTrading.py Trade cycle failed: {e}', traceback_exception)
                    break
            else:
                logging.warning(f'VictorTrading.py Entry time has passed : {message}')
    ...
    def parse_trading_signal(message: str) -> tuple[Optional[str], Optional[str], Optional[str]]:
        ...
        if not re.search(r'Martingale levels', message, re.IGNORECASE) or re.search(r'MEANING OF OUR SIGNALS', message, re.IGNORECASE):
    ...
    async def execute_trade_cycle(self, channel: str, retry: int) -> Optional[bool]:
        ...
        logging.info(f'VictorTrading.py {martingale_message}INITIATE TRADE')
        ...
        logging.info(f'VictorTrading.py {martingale_message}TRADE PLACED SUCCESSFULLY: "{trade_id}" AND WAITING FOR RESPONSE')

        expiration = int(self.pocket_option.get_value(channel, 'expiration')) - 5
        logging.info(f'VictorTrading.py Waiting for the expiration time minus 5 seconds : {expiration}')
        ...
        logging.info(f'VictorTrading.py Trade result: {trade_result}')

        if trade_result in ['win', 'draw']:
            logging.info(f'VictorTrading.py Trade successful: {trade_result}')
            self.pocket_option.remove_channel_data(channel)
            return True
        ...
        logging.warning(f'VictorTrading.py {martingale_message}Trade {trade_id} failed ({trade_result}){message_retrying}')
        return False

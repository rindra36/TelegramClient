...
class GavinBotM1TradingBot:
    ...
    async def handle_trade_execution(self, message: str) -> bool:
        ...
        payout = await self.pocket_option.get_payout(asset)

        if payout < 80:
            logging.info(f"GavinBotM1Trading.py Payout is less than 80% ({payout}%) on {asset}. Asking ...")
            continue_action = input("Do you want to continue ? [y|n]")

            if continue_action.lower() != 'y':
                return False

        logging.info(f'GavinBotM1Trading.py Payout is {payout}% on {asset}')
        ...
        logging.info(f"GavinBotM1Trading.py Trade parameters: {self.pocket_option.get_channel_data(channel)}")
            ...
            if retry > 0 or await waiting_time(entry, timezone_offset=self.TIMEZONE_OFFSET):
                try:
                    ...
                    if not self.DO_MARTINGALE:
                        logging.warning(f'GavinBotM1Trading.py Martingale needed here but not activated. Stop on retry {retry}')
                        return False
                    retry += 1
                    amount *= 2
                    ...
                    logging.error(f'GavinBotM1Trading.py Trade cycle failed: {e}', traceback_exception)
                    break
            else:
                logging.warning(f'GavinBotM1Trading.py Entry time has passed : {message}')
    ...
    def parse_trading_signal(message: str) -> tuple[Optional[str], Optional[str], Optional[str]]:
        ...
        if not re.search(r'POCKET OPTION \[M1\]', message, re.IGNORECASE):
            return action, asset, entry

        # Remove "POCKET OPTION [M1]" from message
        message = re.sub(r'POCKET OPTION \[M1\]', '', message, flags=re.IGNORECASE)
        ...
        if asset_match:
            has_otc = False
            if asset_match.group(3):
                has_otc = True

            asset = f'{asset_match.group(1)}{asset_match.group(2)}'
            if has_otc:
                asset += '_otc'
    ...
    async def execute_trade_cycle(self, channel: str, retry: int) -> Optional[bool]:
        ...
        logging.info(f'GavinBotM1Trading.py {martingale_message}INITIATE TRADE')
        ...
        logging.info(f'GavinBotM1Trading.py {martingale_message}TRADE PLACED SUCCESSFULLY: "{trade_id}" AND WAITING FOR RESPONSE')

        expiration = int(self.pocket_option.get_value(channel, 'expiration')) - 5
        logging.info(f'GavinBotM1Trading.py Waiting for the expiration time minus 5 seconds : {expiration}')
        ...
        logging.info(f'GavinBotM1Trading.py Trade result: {trade_result}')

        if trade_result in ['win', 'draw']:
            logging.info(f'GavinBotM1Trading.py Trade successful: {trade_result}')
            self.pocket_option.remove_channel_data(channel)
            return True
        ...
        logging.warning(f'GavinBotM1Trading.py {martingale_message}Trade {trade_id} failed ({trade_result}){message_retrying}')
        return False

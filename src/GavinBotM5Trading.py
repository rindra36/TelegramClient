...
class GavinBotM5TradingBot:
    ...
    async def handle_trade_execution(self, message: str) -> bool:
        ...
        if payout < 80:
            logging.info(f"GavinBotM5Trading.py Trade skipped. Payout is less than 80% ({payout}%) on {asset}")
            return False

        logging.info(f'GavinBotM5Trading.py Payout is {payout}% on {asset}')
        ...
        logging.info(f"GavinBotM5Trading.py Trade parameters: {self.pocket_option.get_channel_data(channel)}")
            ...
            if retry > 0 or await waiting_time(entry, timezone_offset=self.TIMEZONE_OFFSET):
                try:
                    ...
                    if not self.DO_MARTINGALE:
                        logging.warning(f'GavinBotM5Trading.py Martingale needed here but not activated. Stop on retry {retry}')
                        return False
                    retry += 1
                    amount *= 2
                    ...
                    logging.error(f'GavinBotM5Trading.py Trade cycle failed: {e}', traceback_exception)
                    break
            else:
                logging.warning(f'GavinBotM5Trading.py Entry time has passed : {message}')
    ...
    def parse_trading_signal(message: str) -> tuple[Optional[str], Optional[str], Optional[str]]:
        ...
        if not re.search(r'POCKET OPTION \[M5\]', message, re.IGNORECASE):
            return action, asset, entry

        # Remove "POCKET OPTION [M5]" from message
        message = re.sub(r'POCKET OPTION \[M5\]', '', message, flags=re.IGNORECASE)
    ...
    async def execute_trade_cycle(self, channel: str, retry: int) -> Optional[bool]:
        ...
        logging.info(f'GavinBotM5Trading.py {martingale_message}INITIATE TRADE')
        ...
        logging.info(f'GavinBotM5Trading.py {martingale_message}TRADE PLACED SUCCESSFULLY: "{trade_id}" AND WAITING FOR RESPONSE')

        expiration = int(self.pocket_option.get_value(channel, 'expiration')) - 5
        logging.info(f'GavinBotM5Trading.py Waiting for the expiration time minus 5 seconds : {expiration}')
        ...
        logging.info(f'GavinBotM5Trading.py Trade result: {trade_result}')

        if trade_result in ['win', 'draw']:
            logging.info(f'GavinBotM5Trading.py Trade successful: {trade_result}')
            self.pocket_option.remove_channel_data(channel)
            return True
        ...
        logging.warning(f'GavinBotM5Trading.py {martingale_message}Trade {trade_id} failed ({trade_result}){message_retrying}')
        return False

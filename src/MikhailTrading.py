...
from src.utils import waiting_time, safe_trade, \
    find_trade_in_opened_deals, get_trade_result, determine_trade_result, check_other_asset


class MikhailTradingBot:
    """
    ...
    async def handle_trade_execution(self, message: str) -> bool:
        ...
        logging.info(f"MikhailTrading.py Trade parameters: {self.pocket_option.get_channel_data(channel)}")
            ...
            if retry > 0 or await waiting_time(entry, timezone_offset=self.TIMEZONE_OFFSET):
                try:
                    ...
                    if not self.DO_MARTINGALE:
                        logging.warning(f'MikhailTrading.py Martingale needed here but not activated. Stop on retry {retry}')
                        return False
                    retry += 1
                    amount *= 2
                    ...
                    logging.error(f'MikhailTrading.py Trade cycle failed: {e}', traceback_exception)
                    break
            else:
                logging.warning(f'MikhailTrading.py Entry time has passed : {message}')
    ...
    async def execute_trade_cycle(self, channel: str, retry: int) -> Optional[bool]:
        ...
        logging.info(f'MikhailTrading.py {martingale_message}INITIATE TRADE')
        ...
        logging.info(f'MikhailTrading.py {martingale_message}TRADE PLACED SUCCESSFULLY: "{trade_id}" AND WAITING FOR RESPONSE')

        expiration = int(self.pocket_option.get_value(channel, 'expiration')) - 5
        logging.info(f'MikhailTrading.py Waiting for the expiration time minus 5 seconds : {expiration}')
        ...
        logging.info(f'MikhailTrading.py Trade result: {trade_result}')

        if trade_result in ['win', 'draw']:
            logging.info(f'MikhailTrading.py Trade successful: {trade_result}')
            self.pocket_option.remove_channel_data(channel)
            return True
        ...
        logging.warning(f'MikhailTrading.py {martingale_message}Trade {trade_id} failed ({trade_result}){message_retrying}')
        return False

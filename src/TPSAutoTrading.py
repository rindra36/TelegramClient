...
class TPSAutoTradingBot:
    ...
    def __init__(self, pocket_option):
        ...
        self.SESSION_NAME = 'TPSAuto'
        self.TRADE_PATTERNS = {
            'ASSET_EXPIRATION_ACTION': r'(\w{6})(?:\s+)?(?:OTC\s+)?(\d+)(?:\s+)?MINUTES(?:\s+)(UP|DOWN)',
        }
    ...
    async def handle_trade_execution(self, message: str) -> bool:
        ...
        if not self._has_complete_trade_data(channel):
            ...
            logging.info(f'TPSAutoTrading.py {extracted_data}')
    ...
    async def _execute_new_trade(self, channel: str) -> bool:
        ...
        logging.info(f'TPSAutoTrading.py Trade parameters: {self.pocket_option.get_channel_data(channel)}')

        try:
            return await self.execute_trade_cycle(channel)
        except Exception as e:
            logging.error(f'TPSAutoTrading.py Trade cycle failed: {e}')
            return False
    ...
    def parse_trading_signal(self, message: str) -> Dict[str, str] | bool:
        ...
        if asset_expiration_action_match:
            ...
            action = None
            if asset_expiration_action_match.group(3).upper() == 'UP':
                action = 'BUY'
            elif asset_expiration_action_match.group(3).upper() == 'DOWN':
                action = 'SELL'

            asset = self.normalize_currency_code(f'{asset_expiration_action_match.group(1)}{has_otc}')

            data.extend([
                {'key': 'asset', 'value': asset},
                {'key': 'expiration', 'value': int(asset_expiration_action_match.group(2)) * 60},
    ...
    async def execute_trade_cycle(self, channel: str) -> bool:
        ...
        """
        logging.info('TPSAutoTrading.py INITIATE TRADE')
        ...
        self.pocket_option.remove_channel_data(channel)
        logging.info(f'TPSAutoTrading.py TRADE PLACED SUCCESSFULLY: "{trade_id}"')
    ...
    async def _monitor_trade_result(self, trade_id: str, channel: str) -> bool:
        ...
        logging.info(f'TPSAutoTrading.py Trade result: {trade_result}')

        if trade_result in ['win', 'draw']:
            logging.info(f'TPSAutoTrading.py Trade successful: {trade_result}')
            return True

        logging.warning(f'TPSAutoTrading.py Trade {trade_id} failed ({trade_result})')
        return False
